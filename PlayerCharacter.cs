...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        private float _dashingVelocity = 0.3f;
        private float _dashingTime = 0.6f;
        Vector3 _dashingDir;
        bool _isDashing;
        bool _canDash = true;
        ...
        {
          if (CheckForDashInput() && _canDash)
          {
                _isDashing = true;
                _canDash = false;
                _dashingDir = new Vector3(PlayerInput.Instance.Horizontal.Value, PlayerInput.Instance.Vertical.Value, 0);
                if (_dashingDir == Vector3.zero)
                {
                    _dashingDir = new Vector3(transform.localScale.x, 0, 0);
                }
